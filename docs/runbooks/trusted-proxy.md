...
(§5.1 clarification — `clientIp()` prefers the trusted `cf-connecting-ip` header,
falling back to the untrusted first `X-Forwarded-For` hop).
...
`clientIp()` in `apps/app-api/auth/http.ts` derives the caller IP
for the 20/hr per-IP magic-link limit like this:

1. prefer the **trusted** `cf-connecting-ip` header (set by Cloudflare, unforgeable
   by the client), else
2. fall back to the **FIRST** hop of `X-Forwarded-For` (`xff.split(",")[0]`) —
   treated as **untrusted**, else
3. bucket the caller as the literal string `"unknown"`.

Preferring `cf-connecting-ip` keeps the limiter key stable under a rotating
spoofed `X-Forwarded-For`. But `X-Forwarded-For` is still a client-supplied
request header, and its first hop is only the real client IP when a trusted proxy
has **rewritten** the header. Two failure modes remain if the invariant is
violated *and* `cf-connecting-ip` is not set:

- **Spoofing.** If app-api is reachable directly (or the edge *appends* to a
  client-supplied XFF instead of replacing it) and no `cf-connecting-ip` is
  present, an attacker sets `X-Forwarded-For: 1.2.3.4` per request and rotates it
  freely. Each forged value is a fresh per-IP bucket, so the **20/hr** per-IP
  magic-link cap is bypassed at will — unlimited magic-link emails to any address.
  In the v1 Cloudflare topology this is closed because `cf-connecting-ip` is
  always present and wins over XFF.
- **Bucket collapse.** Direct callers with no `cf-connecting-ip` and no XFF all
