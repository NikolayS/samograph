#!/usr/bin/env bash
# Record a REAL Claude Code session for the samograph demo.
#
# This captures an actual `claude` TUI with asciinema — real typing, real LLM
# streaming, real samograph output. No simulation. You drive the keyboard;
# follow demo/PROMPTS.md for the scripted turns.
#
#   ./demo/record-live.sh                 # → demo/samograph-live.cast
...
# never printed by samograph or claude. DO NOT run `env`, `export`, or `echo
# $RECALL_API_KEY` while recording. The prompt sheet keeps the key off-screen.
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"
OUT="${1:-$SCRIPT_DIR/samograph-live.cast}"
...
asciinema rec "$OUT" \
  --idle-time-limit 3 \
  --title "samograph — put your AI agent in the meeting" \
  --command "claude"
