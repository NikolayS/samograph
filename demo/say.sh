#!/usr/bin/env bash
# Append ONE transcript line to the file that the active `samoagent watch` is
...
# write state.json, so it is safe to run against a REAL `samoagent join` session:
...
#
# It resolves the transcript file from the live state (SAMOAGENT_STATE_FILE or
# ~/.samoagent/state.json), falling back to ~/.samoagent/transcript.txt.
set -euo pipefail
...
STATE="${SAMOAGENT_STATE_FILE:-$HOME/.samoagent/state.json}"
TF="$(awk -F'"' '/"transcript_file"/{print $4; exit}' "$STATE" 2>/dev/null || true)"
[ -n "$TF" ] || TF="$HOME/.samoagent/transcript.txt"
