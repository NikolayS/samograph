...
- **systemd**: `samograph-web` (ExecStart=`/opt/samograph/start-prod.sh` → `bun apps/app-api/server.ts` bg + `bun run start` in apps/web) and `samograph-live` (ExecStart=`bun apps/ws-hub/dev-live-server.ts` — ingest + ws-hub in one process; Bun SQL has no cross-process LISTEN, so they share an in-process Hub). Both confirmed READ-ONLY via `systemctl cat` 2026-07-10. `start-preview.sh` (the old dev-launcher) is superseded by `.samohost.toml` `execStart` fields and should be removed from `/opt/samograph/` on next hostprep.
  - **INFRA REPOINT DONE (#105/#64):** `start-prod.sh` now launches `apps/app-api/server.ts` with `SAMO_ENV=prod` + real secrets. Session cookies carry `Secure`; `GET /__dev/last-magic-link` → 404; boot logs confirm no dev-default-secret warning.
