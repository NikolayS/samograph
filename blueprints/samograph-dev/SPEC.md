...
> **Amendment (S3-1).** The functional requirement here is *"one static public HTTPS `/webhook` per region with per-call routing in `?bot=&t=`"* — the **named tunnel is a dev-reachability artifact and an OPTIONAL prod security posture (zero inbound ports + CF edge), not a hard prod prerequisite**. A plain public HTTPS `/webhook` (Caddy → ingest) is equally correct. See `SPEC.amendments.md` → S3-1.
...
> **Amendment (S3-1).** Read `https://<regional-tunnel>/health` as *"the region's public `/health` marker route"* — the watchdog probes whatever public HTTPS ingress the region uses (Caddy → ingest today, S2-15), whether or not that ingress sits behind a named tunnel. The named tunnel is optional posture, not a functional prerequisite. See `SPEC.amendments.md` → S3-1 (and S2-15).
...
> **Amendment (S3-1).** The named tunnel is an **optional** ingress posture (zero inbound ports + CF edge), consistent with this section's own "fail over to a second tunnel/`--webhook-base`" and "keep Cloudflare off the critical live path". The functional prod requirement is a public HTTPS `/webhook`; the tunnel is not a hard dependency. See `SPEC.amendments.md` → S3-1.
