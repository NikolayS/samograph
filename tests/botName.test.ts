...
describe("botName", () => {
  it("with agent name", () => {
    expect(botName("TARS")).toBe("TARS \u{1F534} (samograph)");
  });

  it("without agent name (null)", () => {
    expect(botName(null)).toBe("samograph \u{1F534}");
  });

  it("without agent name (empty string)", () => {
    // empty string is falsy — same as null
    expect(botName("")).toBe("samograph \u{1F534}");
  });
  ...
  it("exact boundary name not truncated", () => {
    const suffix = " \u{1F534} (samograph)"; // 13 code points
